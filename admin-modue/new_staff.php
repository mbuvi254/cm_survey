		...
		<div class="card-body">
			<form action="" id="manage_staff">
				...
				<div class="row">
					<div class="col-md-6 border-right">
						...
						<select name="county" id="county" class="form-control" required>
								<option value="<?php echo isset($county) ? $county : '' ?>">County:<?php echo isset($county) ? $county : '' ?></option>
					...
					<div class="form-group">
							...
							<input type="email" class="form-control form-control" name="email" required value="<?php echo isset($email) ? $email : '' ?>">
							<small id="#msg"></small>
						</div>
						<div class="form-group">
				...
				<div class="card-footer">
					...
					<button class="btn btn-secondary btn-flat btn-block" type="button" onclick="location.href = 'index.php?page=staff_list'">Cancel</button>
				</div>
			</form>
		</div>
	</div>
	...
	$('#manage_staff').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast('Staff successfully Registered.',"success");
					setTimeout(function(){
