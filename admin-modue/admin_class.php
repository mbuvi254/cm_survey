...
Class Action {
	...
	function save_county(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO county set $data");
		}else{
			$save = $this->db->query("UPDATE county set $data where id = $id");
		}
	...
	function delete_county(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM county where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_survey(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO survey set $data");
		}else{
			$save = $this->db->query("UPDATE survey set $data where id = $id");
		}
